## Robert J. Myers — Identity Governance & Access Management (IAM)

20-year U.S. Navy veteran with experience in identity governance, access accountability, compliance operations, and regulated-environment security workflows. Building hands-on IAM, Microsoft Entra ID, and governance solutions focused on Zero Trust, audit readiness, and operational security.

Currently building practical IAM governance labs, RBAC models, access review workflows, Conditional Access policies, and audit-ready evidence demonstrations aligned to NIST 800-53 and CMMC Level 2.

---

### 🎯 Featured Work

## IAM Proof Portfolio — Microsoft Entra ID
**[IAM Proof Portfolio →](https://github.com/RobertMyersCloud/iam-proof-portfolio)**

Hands-on identity governance portfolio demonstrating practical IAM and IGA workflows including:

Joiner-Mover-Leaver (JML) lifecycle management
Access Reviews & Certification Campaigns
RBAC & Least Privilege Enforcement
Privileged Identity Management (PIM)
Conditional Access Governance
Segregation of Duties (SoD)
PowerShell & Microsoft Graph automation
Audit-ready governance evidence aligned to NIST & CMMC

---

### 🛠️ Identity & Governance Stack

**Identity & Access Management:** Microsoft Entra ID · Microsoft Entra ID Governance · Microsoft Entra PIM · Conditional Access · Okta Identity Concepts · SailPoint Identity Concepts · Active Directory · LDAP · SAML · OAuth · OIDC · SCIM · CAC/PKI

**Governance & Compliance:** NIST 800-53 Rev 5 · NIST 800-171 · CMMC 2.0 · SOC 2 Type II · Zero Trust (CISA ZTMM) · DFARS · Audit Readiness & Evidence Governance

**Technical & Automation:** PowerShell · Microsoft Graph · Azure · Python · Terraform Fundamentals · Bash · SQL

---

### 📜 Certifications

**Identity, Governance & Security:** SailPoint Identity Security Leader · Okta Identity Foundations · SC-900 — Security, Compliance & Identity Fundamentals · Security+ · CMMC RP (CyberAB) · CCZT (Zero Trust)  (in progress: SC-300, Okta Certified Professional, Okta Certified Administrator)

**Cloud & Infrastructure:** ITIL 4 · AZ-900 · AWS Certified Cloud Practitioner · Network+ · FinOps Certified Practitioner · ISC2 CC · Fortinet Associate · Fortinet Fundamentals · Scrum Master · Lean Six Sigma Green Belt (in progress: AZ-500) 

---

### 🎖️ Background

**U.S. Navy** (2001-2021) — U.S. Navy veteran (2001–2021) with operational leadership experience across identity accountability, personnel security, audit readiness, and access governance operations supporting more than 3,500 personnel and 2,200+ CAC/PKI credentials in regulated federal environments.

**Post-Navy civilian:** Senior Production Buyer at Mammoth Freighters (aerospace/defense-adjacent). Supply Chain Manager at Aero Components LLC (DFARS/ITAR, $10M spend). Warehouse Foreman at Federal Bureau of Prisons (DOJ High-Risk Public Trust).

**Education:** MBA — University of the Incarnate Word (GPA 3.85, High Distinction) · BS Accounting — Colorado Technical University (Summa Cum Laude)

**Clearance:** Prior U.S. Government Secret · DOJ (LEO) Public Trust

---

### 📬 Contact

- 📧 robert.j.myers5@gmail.com    
- 💼 [LinkedIn](https://linkedin.com/in/robertjmyers5)

---

*Weatherford, TX · Last updated May 2026*
